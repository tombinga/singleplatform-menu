...
$currency    = isset($data['currency']) ? $data['currency'] : (isset($view['currency']) ? $view['currency'] : 'USD');

$attrs = function_exists('get_block_wrapper_attributes') ? get_block_wrapper_attributes(array('class' => 'sp-menu')) : 'class="sp-menu"';
?>
<section <?php echo $attrs; ?>>
  <h2><?php echo esc_html( isset($data['menuName']) ? $data['menuName'] : (isset($data['locationName']) ? $data['locationName'] : __('Menu','sp-menu')) ); ?></h2>

  <?php if (!empty($data['categories'])): foreach ($data['categories'] as $i => $cat):
    $cid = 'sp-cat-' . (int) $i; ?>
  ...
  <?php if (!empty($data['footnote'])): ?>
    <p class="sp-menu__footnote"><small><?php echo wp_kses_post($data['footnote']); ?></small></p>
  <?php endif; ?>

  <?php if (!empty($data['attribution']['img'])): ?>
    <p class="sp-menu__attrib">
      <a href="<?php echo esc_url($data['attribution']['href']); ?>" rel="nofollow noopener" target="_blank">
        <img src="<?php echo esc_url($data['attribution']['img']); ?>" alt="<?php echo esc_attr__('Provided by SinglePlatform', 'sp-menu'); ?>" loading="lazy" />
      </a>
    </p>
  <?php endif; ?>
</section>
